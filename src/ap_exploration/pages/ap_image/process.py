...
def extract_ap_from_file(document_id: str) -> None:
    dump_ap_extraction_step(APExtractionStep(f'Starting OCR.', 0.05, False), document_id)

    def _callback(advancement: float) -> None:
        advancement = advancement * 0.9
        dump_ap_extraction_step(APExtractionStep(f'{advancement*100:.2f}%', advancement, False), document_id)
...
if __name__ == '__main__':
    ...
    elif args.mode == _AP_EXTRACTION:
        extract_ap_from_file(args.doc)
    else:
