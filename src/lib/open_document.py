...
def _extract_title(tag: bs4.Tag) -> ODFTitle:
    level_str = tag.attrs.get('outline-level')
    if not isinstance(level_str, str) or not level_str.isdigit():
        raise ValueError(f'Expecting "outline-level" attribute to be a string digit, received: {level_str}')
    return ODFTitle(str(tag.string), int(level_str))
...
def _extract_string_from_tag(tag: Any) -> str:
    ...
    if isinstance(tag, bs4.NavigableString):
        return str(tag)
    if tag.name == ODFXMLTagNames.TEXT_P.value:
        return _extract_string_from_tags(list(tag.children)) + '\n'
    return _extract_string_from_tags(list(tag.children))


def _extract_string_from_tags(tags: List[Any]) -> str:
    return ''.join([_extract_string_from_tag(tag) for tag in tags])


def _extract_cell(tag: Any) -> Optional[Cell]:
    if not isinstance(tag, bs4.Tag):
        raise ValueError(f'Expecting tag as input, received element of type {type(tag)}')
    _expected_names = (ODFXMLTagNames.TABLE_CELL.value,)
    if tag not in _expected_names:
        raise ValueError(f'Expecting tag with name in {_expected_names}, tag with name {tag.name}')
    row_span = int(tag.attrs.get(ODFXMLAttributes.TABLE_ROW_SPAN.value, 0))
    col_span = int(tag.attrs.get(ODFXMLAttributes.TABLE_COL_SPAN.value, 0))
...
def _extract_rows(tag: Any, is_header: bool = False) -> List[Row]:
    ...
    if tag not in _expected_names:
        raise ValueError(f'Expecting tag with name in {_expected_names}, tag with name {tag.name}')
    if tag.name == ODFXMLTagNames.TABLE_HEADER.value:
        return [row for child in tag.children for row in _extract_rows(child, True)]
    if tag.name == ODFXMLTagNames.TABLE_COLUMN.value:
...
def _extract_table(tag: bs4.Tag) -> Table:
    if tag.name != ODFXMLTagNames.TABLE_TABLE.value:
        raise ValueError(f'Expecting table tag, received tag with name {tag.name}')
    rows: List[Row] = []
...
_Element = Union[Table, str, ODFTitle]


def _extract_list_item_text(tag: bs4.Tag) -> str:
    if tag.name != ODFXMLTagNames.TEXT_LIST_ITEM.value:
        raise ValueError(f'Expecting text:list-item tag, received tag with name {tag.name}')
    return _extract_string_from_tag(tag)


def _extract_list_elements(tag: bs4.Tag) -> List[str]:
    if tag.name != ODFXMLTagNames.TEXT_LIST.value:
        raise ValueError(f'Expecting text:list tag, received tag with name {tag.name}')
    results: List[str] = []
    for child in tag.children:
...
def _extract_flattened_elements(tag: Any) -> List[_Element]:
    ...
    if tag.name == ODFXMLTagNames.TEXT_H.value:
...
def _build_structured_text_from_soup(tag: bs4.Tag) -> StructuredText:
    ...
    return _build_structured_text(None, elements)
