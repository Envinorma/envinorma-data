...
class InapplicableSectionOld:
    ...
    @classmethod
    def from_dict(cls, dict_: Dict[str, Any]) -> 'InapplicableSectionOld':
        return InapplicableSectionOld(
...
class AlternativeSectionOld:
    ...
    @classmethod
    def from_dict(cls, dict_: Dict[str, Any]) -> 'AlternativeSectionOld':
        return AlternativeSectionOld(
...
class AMWarningOld:
    ...
    @classmethod
    def from_dict(cls, dict_: Dict[str, Any]) -> 'AMWarningOld':
        return AMWarningOld(SectionReference.from_dict(dict_['targeted_section']), dict_['text'])


def extract_conditions_from_parametrization(
    parameter: Parameter, parametrization: 'ParametrizationOld'
) -> List[LeafCondition]:
...
def _check_consistency_on_section(
    inapplicable_sections: List[InapplicableSectionOld], alternative_sections: List[AlternativeSectionOld]
) -> None:
...
class ParametrizationOld:
    ...
    alternative_sections: List[AlternativeSectionOld]
    warnings: List[AMWarningOld]
    path_to_conditions: Dict[Ints, List[InapplicableSectionOld]] = field(init=False)
    path_to_alternative_sections: Dict[Ints, List[AlternativeSectionOld]] = field(init=False)
    path_to_warnings: Dict[Ints, List[AMWarningOld]] = field(init=False)
    ...
    @classmethod
    def from_dict(cls, dict_: Dict[str, Any]) -> 'ParametrizationOld':
        # field renaming
        ...
        return ParametrizationOld(
            [InapplicableSectionOld.from_dict(app) for app in inapplicable_sections_raw],
            [AlternativeSectionOld.from_dict(sec) for sec in dict_['alternative_sections']],
            [AMWarningOld.from_dict(sec) for sec in dict_.get('warnings', [])],
        )
