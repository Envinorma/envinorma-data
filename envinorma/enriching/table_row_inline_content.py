...
def _build_texts_for_row_inline_content(headers: List[str], rows: List[Row]) -> List[str]:
...
def _add_inline_content_in_table_rows(string: EnrichedString) -> EnrichedString:
    table = string.table
    if not table:
        return string
    headers = _extract_headers(table.rows)
    top_header_rows, other_rows = _split_rows(table.rows)
    texts = [''] * len(top_header_rows) + _build_texts_for_row_inline_content(headers, other_rows)
    new_rows = [
        replace(row, inline_content=texts[row_rank]) if not row.is_header else row
        for row_rank, row in enumerate(table.rows)
    ]
    return replace(string, table=replace(table, rows=new_rows))


def _add_table_row_inline_content_data_in_section(section: StructuredText) -> StructuredText:
    section = copy(section)
    section.outer_alineas = [_add_inline_content_in_table_rows(alinea) for alinea in section.outer_alineas]
    section.sections = [_add_table_row_inline_content_data_in_section(subsection) for subsection in section.sections]
    return section


def add_table_row_inline_content(am: ArreteMinisteriel) -> ArreteMinisteriel:
    return replace(
        am, sections=[_add_table_row_inline_content_data_in_section(subsection) for subsection in am.sections]
    )
